  ...
  useEffect(() => {
    const fetchBooks = async () => {
      ...
      if (query && router.isReady) {
        if (windowWidth < 500) {
          endpoint = `http://localhost:3002/hotsearch/rwd`;
        } else {
        ...
        endpoint = windowWidth < 500 ? "http://localhost:3002/oldbook" : `http://localhost:3002/perpage?page=${currentPage}`;
      }
      ...
      try {
        ...
        if (windowWidth < 500 || b_genre_id || book_language || (query && router.isReady)) {
...
}, [b_genre_id, book_language, query, router.isReady, currentPage, sortOrder, windowWidth]);
          ...
          <div className='row '>
            {oldbook && oldbook.length > 0 ? (
              oldbook.map((v) => {
                ...
                return (
                  <div className='col-6 col-lg-2 ' key={v.book_id}>
                    <div
